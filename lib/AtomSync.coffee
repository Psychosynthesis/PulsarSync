...
getLogger = ->
  if not logger
    ...
    logger = new Logger "Atom Sync Log"
  return logger
...
class AtomSync
  ...
  monitorFile: (dirPath, toggle = true, notifications = true)->
    ...
    if dirPath not in MonitoredFiles
      ...
      watcher.add(dirPath)
      if notifications
        atom.notifications.addInfo "atom-sync: Watching file - *"+fileName+"*"
      ...
      if notifications
        atom.notifications.addInfo "atom-sync: Unwatching file - *"+fileName+"*"
    @.monitorStyles()
  ...
  monitorStyles: ()->
    ...
    for file in MonitoredFiles
      ...
      if icon_file != null
        ...
        if atom.config.get("atom-sync.monitorFileAnimation")
  ...
  monitorFilesList: ()->
    ...
    for k,v of watchedPaths
      for file in watchedPaths[k]
        files += file+"<br/>"
    if files != ""
      atom.notifications.addInfo "atom-sync: Currently watching:<br/>*"+files+"*"
    else
      atom.notifications.addWarning "atom-sync: Currently not watching any files"
  ...
  fileExists: (dirPath) ->
    ...
    try
      ...
      atom.notifications.addWarning "atom-sync: cannot find *"+file_name+"* to watch"
      return false

  isDirectory: (dirPath) ->
    if directory = fs.statSync(dirPath).isDirectory()
      atom.notifications.addWarning "atom-sync: cannot watch directory - *"+dirPath+"*"
      return false
  ...
  createTransport: (host)->
    if host.transport is 'scp' or host.transport is 'sftp'
      ...
      throw new Error("[atom-sync] invalid transport: " + host.transport + " in " + @configPath)
...
module.exports =
  create: (projectPath)->
    configPath = path.join projectPath, atom.config.get('atom-sync.configFileName')
    return unless fs.existsSync configPath
    return new AtomSync(projectPath, configPath)
